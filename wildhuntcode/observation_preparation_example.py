...
if __name__ == '__main__':
    ...
    df = pd.read_csv('lofar_sel2.csv')

    # Load example catalog
    cat = whcat.Catalog('qso_candidates',
                        ra_column_name='RA',
                        dec_column_name='DEC',
                        id_column_name='Source_Name',
                        table_data=df)
    ...
    quality_query = "mag_r > 15 AND mag_r < 21 AND type = 'PSF' "
    #20.5 AND type = 'PSF'

    cat.get_offset_stars_datalab(100, datalab_dict=datalab_dict,n=1,
                                 where=quality_query)
    ...
    #offsets = pd.read_csv('qso_candidates_ls_dr9_OFFSETS_check.csv')
    #df = pd.read_csv('qso_candidates_ls_dr9_OFFSETS_check.csv')

    offsets = pd.read_csv('qso_candidates_ls_dr9_OFFSETS.csv')
    df = pd.read_csv('qso_candidates_ls_dr9_OFFSETS.csv')
